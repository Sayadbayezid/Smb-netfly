...
var newsLinks = mysqlTable("news_links", {
  id: varchar("id", { length: 36 }).primaryKey(),
  title: text("title").notNull(),
  description: text("description"),
...
async function getAllNewsLinks() {
  const db = await getDb();
  if (!db) return [];
  try {
...
async function getNewsLinkById(id) {
  const db = await getDb();
  if (!db) return void 0;
  try {
...
async function createNewsLink(link) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  try {
...
async function updateNewsLink(id, link) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  try {
...
async function deleteNewsLink(id) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  try {
...
var appRouter = router({
  ...
  blog: router({
    ...
    getById: publicProcedure.input(z2.string()).query(({ input }) => getBlogPostById(input)),
    create: protectedProcedure.input(z2.object({
      title: z2.string(),
      description: z2.string().optional(),
      content: z2.string(),
      image: z2.string().optional(),
      category: z2.string().default("Personal"),
    ...
    delete: protectedProcedure.input(z2.string()).mutation(({ input }) => deleteBlogPost(input))
  }),
  news: router({
    list: publicProcedure.query(() => getAllNewsLinks()),
    getById: publicProcedure.input(z2.string()).query(({ input }) => getNewsLinkById(input)),
    create: protectedProcedure.input(z2.object({
      title: z2.string(),
      description: z2.string().optional(),
      url: z2.string(),
      category: z2.string().default("Other"),
    ...
    delete: protectedProcedure.input(z2.string()).mutation(({ input }) => deleteNewsLink(input))
  })
});
